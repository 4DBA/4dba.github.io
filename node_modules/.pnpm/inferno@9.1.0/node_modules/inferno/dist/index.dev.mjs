...
function DEV_VALIDATE_KEYS(vNodeTree, childKeys) {
  ...
  const foundKeys = {};
  const forceKeyed = (childKeys & 8 /* ChildFlags.HasKeyedChildren */) !== 0;
  let foundKeyCount = 0;
...
function validateKeys(vNode) {
  {
    ...
    if (!vNode.isValidated && vNode.children && vNode.flags & 481 /* VNodeFlags.Element */) {
      const error = DEV_VALIDATE_KEYS(Array.isArray(vNode.children) ? vNode.children : [vNode.children], vNode.childFlags);
      if (error) {
...
{
  const skipWarnings = typeof SKIP_INFERNO_WARNINGS !== 'undefined' || typeof process === 'object' && (process.env?.SKIP_INFERNO_WARNINGS !== undefined || process.env?.JEST_WORKER_ID !== undefined);
  if (!skipWarnings) {
