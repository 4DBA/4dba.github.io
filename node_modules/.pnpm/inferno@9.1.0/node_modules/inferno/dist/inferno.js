(function (global, factory) {
    ...
    function getTagName(input) {
      ...
      if (isArray(input)) {
        ...
        var flags = input == null ? void 0 : input.flags;
        if (!isNumber(flags)) {
          try {
    ...
    function DEV_VALIDATE_KEYS(vNodeTree, childKeys) {
      if ((childKeys & 4 /* ChildFlags.HasNonKeyedChildren */) !== 0) {
        return null;
      }
      var foundKeys = {};
      var forceKeyed = (childKeys & 8 /* ChildFlags.HasKeyedChildren */) !== 0;
      var foundKeyCount = 0;
    ...
    function validateKeys(vNode) {
      {
        ...
        if (!vNode.isValidated && vNode.children && vNode.flags & 481 /* VNodeFlags.Element */) {
          var error = DEV_VALIDATE_KEYS(Array.isArray(vNode.children) ? vNode.children : [vNode.children], vNode.childFlags);
          if (error) {
    ...
    function getChildFlagsName(childFlags) {
      switch (childFlags) {
        ...
        case 0 /* ChildFlags.UnknownChildren */:
          return 'ChildFlags.UnknownChildren';
        default:
          return "ChildFlags.Unknown(" + childFlags + ")";
      }
    }
    function validateChildFlags(vNode) {
      var childFlags = vNode.childFlags;
      var children = vNode.children;
      var parentTag = getTagName(vNode);
      switch (childFlags) {
        case 0 /* ChildFlags.UnknownChildren */:
        case 1 /* ChildFlags.HasInvalidChildren */:
          return;
        case 16 /* ChildFlags.HasTextChildren */:
          ...
          if (children && isNumber(children.flags) && children.flags & 16 /* VNodeFlags.Text */) {
            throwError(getChildFlagsName(childFlags) + " expects children to be a bare string, not a Text VNode. Location: \n" + getTagName(children) + parentTag);
          }
          throwError(getChildFlagsName(childFlags) + " expects children to be a string. Location: \n" + getTagName(children) + parentTag);
          return;
        case 2 /* ChildFlags.HasVNodeChildren */:
          if (isInvalid(children) || isArray(children) || isStringOrNumber(children)) {
            throwError(getChildFlagsName(childFlags) + " expects children to be a VNode. Location: \n" + getTagName(children) + parentTag);
          }
        ...
        case 8 /* ChildFlags.HasKeyedChildren */:
          if (!isArray(children)) {
            throwError(getChildFlagsName(childFlags) + " expects children to be an array of VNodes. Location: \n" + getTagName(children) + parentTag);
          }
          for (var i = 0; i < children.length; i++) {
            if (!(i in children)) {
              throwError(getChildFlagsName(childFlags) + " expects children to be a flat array without holes; found a hole at index " + i + ". Location: \n" + parentTag);
            }
            var child = children[i];
            if (isArray(child)) {
              throwError(getChildFlagsName(childFlags) + " expects children to be a flat array; found a nested array at index " + i + ". Location: \n" + getTagName(child) + parentTag);
            }
            if (isInvalid(child)) {
              throwError(getChildFlagsName(childFlags) + " expects children to be VNodes; found invalid child at index " + i + ". Location: \n" + getTagName(child) + parentTag);
            }
            if (isStringOrNumber(child)) {
              throwError(getChildFlagsName(childFlags) + " expects children to be VNodes; found text at index " + i + ". Location: \n" + getTagName(child) + parentTag);
            }
            throwIfObjectIsNotVNode(child);
            if (childFlags === 8 /* ChildFlags.HasKeyedChildren */) {
              if (isNullOrUndef(child.key)) {
                throwError(getChildFlagsName(childFlags) + " expects all children to have keys; missing key at index " + i + ". Location: \n" + getTagName(child) + parentTag);
              }
            }
          }
          return;
...

    {
      ...
      var skipWarnings = typeof SKIP_INFERNO_WARNINGS !== 'undefined' || typeof process === 'object' && (((_process$env = process.env) == null ? void 0 : _process$env.SKIP_INFERNO_WARNINGS) !== undefined || ((_process$env2 = process.env) == null ? void 0 : _process$env2.JEST_WORKER_ID) !== undefined);
      if (!skipWarnings) {
